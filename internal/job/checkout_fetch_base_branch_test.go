...
func TestFetchSourceBaseBranch(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := e.fetchSource(t.Context(), false); err != nil {
				t.Fatalf("e.fetchSource(ctx, false) error = %v, want nil", err)
			}
...
func TestFetchSourceBaseBranchUpdatesRefWithoutRefmapAndAfterForcePush(t *testing.T) {
	...
	if err := e.fetchSource(t.Context(), false); err != nil {
		t.Fatalf("e.fetchSource(ctx, false) after force-push error = %v, want nil", err)
	}
